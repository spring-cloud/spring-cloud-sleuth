...
public class ZipkinSpanListener implements SpanReporter {
	...
	private static final List<String> ZIPKIN_CLIENT_EVENTS = Arrays.asList(
			Constants.CLIENT_RECV, Constants.CLIENT_SEND
	);
	private static final List<String> ZIPKIN_SERVER_EVENTS = Arrays.asList(
			Constants.SERVER_RECV, Constants.SERVER_SEND
	);
	...
	private void processLogs(Span span, zipkin.Span.Builder zipkinSpan, Endpoint endpoint) {
		...
		boolean clientEventToTag = false;
		boolean serverEventToTag = false;
		for (Log log : span.logs()) {
			if (ZIPKIN_CLIENT_EVENTS.contains(log.getEvent())) {
				clientEventToTag = true;
			} else if (ZIPKIN_SERVER_EVENTS.contains(log.getEvent())) {
				serverEventToTag = true;
			}
		...
		if (clientEventToTag && this.environment != null) {
			setInstanceIdIfPresent(zipkinSpan, endpoint, Span.SPAN_CLIENT_INSTANCEID);
		}
		if (serverEventToTag && this.environment != null) {
			setInstanceIdIfPresent(zipkinSpan, endpoint, Span.SPAN_SERVER_INSTANCEID);
		}
	}

	private void setInstanceIdIfPresent(zipkin.Span.Builder zipkinSpan,
			Endpoint endpoint, String key) {
		String property = this.environment
				.getProperty("spring.application.instanceid");
