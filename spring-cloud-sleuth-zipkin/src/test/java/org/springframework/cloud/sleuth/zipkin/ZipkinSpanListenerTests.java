...
public class ZipkinSpanListenerTests {
	...
	@Test
	public void shouldAddClientServiceIdTagWhenSpanContainsCS() {
		...
		assertThat(result.binaryAnnotations)
				.filteredOn("key", Span.SPAN_CLIENT_INSTANCEID)
				.extracting(input -> input.value)
				.containsOnly("foo".getBytes());
	}

	@Test
	public void shouldAddClientServiceIdTagWhenSpanContainsCR() {
	...
	@Test
	public void shouldAddServerServiceIdTagWhenSpanContainsSR() {
		this.parent.logEvent(Span.SERVER_RECV);
		this.mockEnvironment.setProperty("spring.application.instanceid", "foo");

		zipkin.Span result = this.spanListener.convert(this.parent);

		assertThat(result.binaryAnnotations)
				.filteredOn("key", Span.SPAN_SERVER_INSTANCEID)
				.extracting(input -> input.value)
				.containsOnly("foo".getBytes());
	}

	@Test
	public void shouldAddServerServiceIdTagWhenSpanContainsSS() {
		this.parent.logEvent(Span.SERVER_SEND);
		this.mockEnvironment.setProperty("spring.application.instanceid", "foo");
	...
	@Test
	public void shouldNotAddAnyServiceIdTagWhenSpanContainsCRAndThereIsNoEnvironment() {
		this.parent.logEvent(Span.CLIENT_RECV);
	...
	@Test
	public void shouldNotAddAnyServiceIdTagWhenSpanContainsSrAndThereIsNoEnvironment() {
		this.parent.logEvent(Span.SERVER_SEND);
		ZipkinSpanListener spanListener = new ZipkinSpanListener(this.spanReporter,
				this.endpointLocator, null);
