...
class TraceMongodbAutoConfigurationTests {
	...
	void should_not_create_reactive_customizer_when_reactive_context_missing() {
		this.contextRunner
				.withClassLoader(new FilteredClassLoader(
						brave.mongodb.MongoDBTracing.class,
						com.mongodb.reactivestreams.client.ReactiveContextProvider.class))
				.run((context) -> assertThat(context)
						.hasSingleBean(TraceMongoClientSettingsBuilderCustomizer.class)
						.doesNotHaveBean(
								TraceReactiveMongoClientSettingsBuilderCustomizer.class));
	}
	...
	void should_create_both_customizers_when_reactive_context_present() {
		this.contextRunner
				.withClassLoader(
						new FilteredClassLoader(brave.mongodb.MongoDBTracing.class))
				.run((context) -> assertThat(context)
						.hasSingleBean(TraceMongoClientSettingsBuilderCustomizer.class)
						.hasSingleBean(
								TraceReactiveMongoClientSettingsBuilderCustomizer.class));
