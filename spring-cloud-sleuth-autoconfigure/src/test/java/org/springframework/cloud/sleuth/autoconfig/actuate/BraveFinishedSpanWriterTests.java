...
class BraveFinishedSpanWriterTests {
	...
	void should_convert_finished_spans_to_zipkin_json() {
		FinishedSpan finishedSpan = new BraveFinishedSpan(new MutableSpan(TraceContext.newBuilder().spanId(1L).traceId(2L).build(), null));

		String json = new BraveFinishedSpanWriter().write(TextOutputFormat.CONTENT_TYPE_OPENZIPKIN_JSON_V2, Collections.singletonList(finishedSpan));

		then(json).isEqualTo("[{\"traceId\":\"0000000000000002\",\"id\":\"0000000000000001\"}]");
	}

	@Test
	void should_return_null_when_unsupported_media_type() {
