...
(sampling), if any remote fields (baggage) and which libraries are traced.
Sleuth also adds annotation based tracing features and some instrumentation not
available otherwise, such as for Reactor.
...
Here are a the most relevant links from the OpenZipkin Brave project:
...
The core `Tracer` type is generally
