...
Essentially with Spring Cloud Span there are two ways span tracing data can be passed to
 a process.
...
As you can see you can disable them with the properties:

* `Message` span extractor - `spring.sleuth.integration.extractor.enabled`
* `MessageBuilder` span injector - `spring.sleuth.integration.injector.enabled`

There is also a WebSockets version:

* `Message` span extractor - `spring.sleuth.integration.websocket.extractor.enabled`
* `MessageBuilder` span injector - `spring.sleuth.integration.websocket.injector.enabled`

You can override them either by disabling with a property and providing your own implementation
or by adding a `@Primary` annotation to your bean definition. Remember to provide the following qualifiers:

* `Message` span extractor - `messagingSpanExtractor`
* `MessageBuilder` span injector - `messagingSpanInjector`
* `Message` span extractor for WebSockets - `stompMessagingSpanExtractor`
* `MessageBuilder` span injector for WebSockets - `stompMessagingSpanInjector`
...
* `HttpServletRequest` span extractor - `spring.sleuth.web.extractor.enabled`
* `HttpServletResponse` span injector - `spring.sleuth.web.injector.enabled`

You can override them either by disabling with a property and providing your own implementation
or by adding a `@Primary` annotation to your bean definition. Remember to provide the following qualifiers:

* `HttpServletRequest` span extractor - `httpServletRequestSpanExtractor`
* `HttpServletResponse` span injector - `httpServletResponseSpanInjector`
