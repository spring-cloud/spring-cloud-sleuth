...
an example of usage of `SkipPatternProvider` inside a `HttpSampler`.

[source,java]
----
include::../../../../spring-cloud-sleuth-core/src/main/java/org/springframework/cloud/sleuth/instrument/web/SleuthHttpSampler.java[lines=30..46]
----
...
Through the `TraceFilter`, all sampled incoming requests result in creation of a Span.
...
If the the `TraceFilter` does not see this attribute, it creates a "`fallback`" span, which is an additional span created on the server side so that the trace is presented properly in the UI.
