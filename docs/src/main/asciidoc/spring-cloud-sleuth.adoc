...
It provides the following wrappers over Feign classes

- `TraceFeignDecoder` - a Feign `Decoder` that closes a Span after decoding is completed
- `TraceFeignRetryer` - a Feign `Retryer` that propagates information that a particular Span was retried
- `TraceFeignRequestInterceptor` - a Feign `RequestInterceptor` that either starts or continues a Span depending on the fact
whether it was retried or not.

In addition to this there is `FeignBeanPostProcessor` that wraps all of the above Feign classes in their Sleuth representative.
 You can disable the post processor by providing the `spring.sleuth.feign.processor.enabled` equal to `false`.

IMPORTANT: If you decide to perform any custom operations with Feign you have to ensure that you're manually wrapping your solutions
in either its Sleuth representation or you write your own one that closes Spans when an execution is finished. Otherwise the tracing
 information propagation can be broken.