...
=== Customization of sent span

In Sleuth we're generating spans with a fixed name. Some users want to modify the name depending on some values
of tags. Implementation of the `SpanAdjuster` interface can be used to alter then name. Example:
