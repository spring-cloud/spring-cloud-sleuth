...
// and method execution
include::../../../../spring-cloud-sleuth-core/src/test/java/org/springframework/cloud/sleuth/annotation/SleuthSpanCreatorAspectTests.java[tags=log_execution,indent=0]
----

If you want to create append a log to the span it's enough to annotate the property of a method
with `@SpanLog` annotation. In this case a log with event `test` will be set on the span.
