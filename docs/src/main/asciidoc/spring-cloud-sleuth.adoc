...
we're actively trying to pass the tracing beanFactory between threads. Also timing events
...
[source,java]
----
include::../../../../spring-cloud-sleuth-core/src/test/java/org/springframework/cloud/sleuth/annotation/SleuthSpanCreatorAspectTests.java[tags=span_log,indent=0]
----

If you want to create append a log to the span it's enough to pass the `log` value of
the annotation. In this case a log with name `test.start` will be set on the span before
the method is executed and a log with name `test.end` just before closing the span.

=== Continuing spans

If you want to just add tags and annotations to an existing span it's enough
to use the `@ContinueSpan` annotation as presented below:
...
That way the span will get continued and:
 ...
 - logs with name `testMethod11.start` and `testMethod11.end` will be created
...
- try with the `SleuthTagValueResolver` bean
- if there is no bean, try to evaluate a SPEL expression
- if there's no SPEL expression just return a `toString()` value of the parameter

==== Custom SleuthTagValueResolver Bean
...
and such a `SleuthTagValueResolver` bean implementation
...
==== Using SpEL expression for value
...
Will lead to evaluation of the SPEL expression and a tag with value `4 characters` will be set on the span.
