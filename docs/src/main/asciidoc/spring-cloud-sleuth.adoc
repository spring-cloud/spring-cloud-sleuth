...
Traces connect from host to host using header propagation. The default format is
https://github.com/openzipkin/b3-propagation[B3]. Similar to data formats, you
can configure alternate header formats also, provided trace and span IDs are
compatible with B3. Most notably, this means the trace ID and span IDs are
lower-case hex, not UUIDs. Besides trace identifiers, other properties (Baggage)
can also be passed along with the request. Remote Baggage must be pre-defined,
but is flexible otherwise.
