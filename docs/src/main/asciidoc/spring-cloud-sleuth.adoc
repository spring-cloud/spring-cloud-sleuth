...
We're registering a custom `HystrixConcurrencyStrategy` that wraps all `Callable` instances into their Sleuth representative -
...
Via the `TraceFilter` all sampled incoming requests result in creation of a Span. That Span's name is `http:` + the path to which
 the request was sent. E.g. if the request was sent to `/foo/bar` then the name will be `http:/foo/bar`.

You can configure which URIs you would like to skip via the `spring.sleuth.instrument.web.skipPattern` property.
...
In order to block the synchronous `RestTemplate` features just set `spring.sleuth.client.enabled` to `false`.
To block the `AsyncRestTemplate` features set `spring.sleuth.async.client.enabled` to `false`.
...
call is made a new Span is created. It gets closed upon receiving the response.

==== Asynchronous Rest Template

Custom instrumentation is set to create and close Spans upon sending and receiving requests.
...
To disable these features just set the `spring.sleuth.integration.enabled` property to `false`.

All http://projects.spring.io/spring-integration/[Spring Integration] based communication gets wrapped in creation / closing of Spans.

Support for Websockets is also implemented.
