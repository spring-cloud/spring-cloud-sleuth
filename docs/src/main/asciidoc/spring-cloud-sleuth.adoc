...
To enable the `ProbabilityBasedSampler` using configuration properties it's enough to set `spring.sleuth.sampler.enabled=true`.
...
A sampler can also be installed by creating a bean definition, as shown in the following example:
