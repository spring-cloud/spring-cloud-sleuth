...
=== Advanced Tag Setting for Method Parameters

There are 3 different ways to add tags to a span to report invocation parameters of a method annotated with `@NewSpan`.
...
=== Advanced Tag Setting for Method Return Values

The return value of a method invocation can be materialized as a tag in the span similarly to the invocation parameters.
The creation of the tag for the return value is controlled by the `SpanTagFromReturnValue` annotation.
The value set to the tag can be calculated in several ways, in order of precedence:
...
If the method has return type `void`, the tag representing the (non-existent) return value will not be created.
If the method returns `null`, the value of the tag will be an empty string.
