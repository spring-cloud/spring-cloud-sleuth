...
TIP: If you are using `spring-cloud-sleuth-stream` and `spring-cloud-netflix-hystrix-stream` (which comes with https://docs.pivotal.io/spring-cloud-services/client-dependencies.html[`spring-cloud-services-starter-circuit-breaker`]) together, Span will be created for each Hystrix metrics and sent to Zipkin. This may be annoying. You can prevent this by setting `spring.sleuth.scheduled.skipPattern=org.springframework.cloud.netflix.hystrix.stream.HystrixStreamTask`
