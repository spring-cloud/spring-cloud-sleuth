...
Part of the Span Context is the Baggage.
...
IMPORTANT: Since you can attach as many baggage elements as you want and it's propagated through the network
then if it's weighing too much or you have too many elements then you it may decrease system throughput or increase RPC latencies.
...
Tags are not passed from a parent to a child, however they are sent to Zipkin and will get presented in the
Zipkin UI for every span.
