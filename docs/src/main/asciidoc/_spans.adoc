...
IMPORTANT: All tags and events must be prefixed with `sql.` prefix!

.Tag Keys
|===
|Name | Description
|sql.datasource.driver|Name of the JDBC datasource driver.
|sql.datasource.pool|Name of the JDBC datasource pool.
|===
...
|===
|Name | Description
|sql.query|The SQL query value.
|sql.row-count|Number of SQL rows.
|===

.Event Values
|===
|Name | Description
|sql.commit|When the transaction gets commited.
|sql.rollback|When the transaction gets rolled back.
|===
