...
|spring.sleuth.jdbc.decorator.datasource.exclude-beans |  | Beans that won't be decorated.
...
|spring.sleuth.jdbc.decorator.datasource.sleuth.enabled | `true` | Creates span for every connection and query. Works only with p6spy or datasource-proxy.
|spring.sleuth.jdbc.decorator.datasource.sleuth.include |  | 
