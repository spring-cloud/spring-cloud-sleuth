 ...
 *
 * <p>
 * Use this when you start traces at the servlet layer via {@link TracingFilter}.
 */
public final class SpanCustomizingHandlerInterceptor implements HandlerInterceptor {
	...
	SpanCustomizingHandlerInterceptor() { // hide the ctor so we can change later if
											// needed
	}
	...
	/**
	 * Sets the "error" attribute if not already set, so that the {@link TracingFilter}
	 * can read it.
	 */
	...
	/**
	 * Sets the "http.route" attribute from {@link #BEST_MATCHING_PATTERN_ATTRIBUTE} so
	 * that the {@link TracingFilter} can read it.
	 */
