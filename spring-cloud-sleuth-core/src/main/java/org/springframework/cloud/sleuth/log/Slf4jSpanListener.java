...
public class Slf4jSpanListener {
	...
	public void start(SpanAcquiredEvent event) {
		...
		MDC.put(Trace.SPAN_ID_NAME, Span.Converter.toHexString(span.getSpanId()));
		MDC.put(Trace.SPAN_EXPORT_NAME, String.valueOf(span.isExportable()));
		MDC.put(Trace.TRACE_ID_NAME, Span.Converter.toHexString(span.getTraceId()));
	...
	public void stop(SpanReleasedEvent event) {
		...
		if (event.getParent() != null) {
			log.trace("With parent: {}", event.getParent());
			MDC.put(Trace.SPAN_ID_NAME, Span.Converter.toHexString(event.getParent().getSpanId()));
